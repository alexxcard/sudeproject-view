...
export default function ProyectosPage() {
  ...
  const addProyecto = () => {
    ...
    const newItem: Proyecto = {
      ...
      members: newProyecto.members ? newProyecto.members.split(",").map(m => m.trim()) : [],
  ...
  return (
    <div className="bg-gray-100 min-h-screen p-6">
      ...
      <div className="max-w-[1200px] mx-auto mb-4 flex justify-end">
        <Button label="Crear Nuevo Proyecto" icon="pi pi-plus" onClick={() => setShowDialog(true)} />
      </div>

      {/* Modal para nuevo proyecto */}
      <Dialog header="Agregar Nuevo Proyecto" visible={showDialog} style={{ width: '400px' }} onHide={() => setShowDialog(false)}>
        <div className="grid grid-cols-1 gap-3">
          ...
          <InputText
            ...
            onChange={(e) => setNewProyecto({ ...newProyecto, owner: e.target.value })}
          />
          <InputText
            placeholder="Miembros (separados por coma)"
            value={newProyecto.members}
            onChange={(e) => setNewProyecto({ ...newProyecto, members: e.target.value })}
          />
          <InputText
        ...
        <Button className="mt-3" label="Agregar Proyecto" onClick={addProyecto} />
      </Dialog>

      {/* Botón volver al inicio */}
      <div className="max-w-[1200px] mx-auto mb-4">
        <Button label="Volver al inicio" icon="pi pi-home" onClick={() => router.push("/home")} />
      </div>
